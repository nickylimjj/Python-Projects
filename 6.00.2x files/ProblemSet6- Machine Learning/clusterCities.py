...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        for p in c2.members():
            points.append(p)
        new_cluster=Cluster(points,type(p))
        self.members.remove(c1), self.members.remove(c2)
        self.add(new_cluster)
    ...
    def findClosest(self, linkage):
        ...
        answer = None
        min_distance = None
        members = self.getClusters()
        for c1 in members:
            for c2 in members:
                if c1==c2:    #prevent self-merge
                    continue
                if answer == None or linkage(c1,c2)<min_distance:
                    answer = (c1,c2)
                    min_distance = linkage(c1,c2)
        return answer
    def mergeOne(self, linkage,toPrint=False):
        ...
        if toPrint:
            print "{} and {} are closest clusters".format(c1,c2)
...
def hCluster(points, linkage, numClusters, printHistory):
    ...
    while cS.numClusters() > numClusters:
        merged = cS.mergeOne(linkage) #change toPrint value print merge process
